...
if parsed_args['render_simple']!=True:
    ...
    def __init__(self, pos, color=(0,125,255), radius=15, is_head=False):
        ...
        if is_head:
            ...
            try:
                ...
                #print("TWO!")
        if self.is_head:
            self.image = pygame.transform.rotate(pygame.transform.scale(self.head_img,(self.radius*2,self.radius*2)),-head_rot)
        else:
            self.image = pygame.transform.rotate(pygame.transform.scale(self.seg_img,(self.radius*2,self.radius*2)),-math.degrees(self.angle))
        ...
        if 'mode' in msg.keys():
            if msg['mode']==0:#why do we receive a handshake now?
                ...
                if 'killer' in msg.keys():
                    text(screen, "You were killed by "+str(msg['killer'])+".", (round(screen.get_width()/2),round(screen.get_height()/2)))
